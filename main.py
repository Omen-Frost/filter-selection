...
prune_epochs = 130
epochs = prune_epochs + warmup # total epochs
prune_interval = 3
start_epoch = 0
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(
    optimizer, milestones=[50, 100], last_epoch=start_epoch - 1)
...
N_layers = 0  # number of conv3d layers in the network
ckp_interval = 5
shape_layers = []
is_pruning = False
...
def solver1(incD):
    ...
    constraints = [cp.sum(z, axis=1) == 1, cp.sum(
        cp.max(z, axis=0)) <= math.ceil(incD.shape[0]*pruning_ratio)]
...
def solver2(incD, incXextD):
    ...
    constraints = [cp.sum(Zn, axis=1) + cp.sum(Zo, axis=1) == 1,
                   cp.sum(cp.max(Zn, axis=0)) <= math.ceil(incD.shape[0]*pruning_ratio)]
    objective = cp.Minimize(
        cp.sum(cp.multiply(incD, Zn)) + cp.sum(cp.multiply(incXextD, Zo)))
    prob = cp.Problem(objective, constraints)
    prob.solve()
...
def prune():
    ...
    for layer in range(0, N_layers):
        dist_mat.append(
            np.zeros([shape_layers[layer][0], shape_layers[layer][0]]))
            ...
            for layer in range(0, N_layers):
                ...
                if device == "cpu":
                    for outputs in features[layer]:
                        ...
                        dist_mat[layer] += distance.cdist(vec,
                                                          vec, "euclidean")
                else:
                    tns = features[layer].flatten(2)
                    dist_mat[layer] += torch.cdist(tns, tns,
                                                   p=2).sum(0).cpu().detach().numpy()
...
def get_features(name):
    def hook(model, input, output):
        ...
        if pruned_idx:
            # zeroize outputs of pruned filters for ALL forward passes
...
for epoch in range(start_epoch, epochs):
    ...
    if epoch >= warmup and (epoch+1) % prune_interval == 0:
